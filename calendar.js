! function(e) { "use strict"; "function" == typeof define && define.amd ? define(["jquery"], e) : "undefined" != typeof exports ? module.exports = e(require("jquery")) : e(jQuery) }(function(d) {
    ...
    i = 0, (r = function(e, t) {
        ...
        a.startingDay = null, a.monthLength = null, a.windowW = d(window).width(), a.$current = { month: isNaN(this.month) || null == this.month ? (new Date).getMonth() : this.month, year: isNaN(this.year) || null == this.year ? (new Date).getFullYear() : this.year, date: a.formatDate(a.initials.dates[a.defaults.language].months[(new Date).getMonth()] + " " + (new Date).getDate() + " " + (new Date).getFullYear(), a.options.format) }, a.$active = { month: a.$current.month, year: a.$current.year, date: a.$current.date, event_date: a.$current.date, events: [] }, a.$label = { days: [], months: a.initials.dates[a.defaults.language].months, days_in_month: [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31] }, a.$markups = { calendarHTML: "", mainHTML: "", sidebarHTML: "", eventHTML: "" }, a.$elements = { calendarEl: d(e), innerEl: null, sidebarEl: null, eventEl: null, sidebarToggler: null, eventListToggler: null, activeDayEl: null, activeMonthEl: null, activeYearEl: null }, a.$breakpoints = { tablet: 768, mobile: 425 }, a.$UI = { hasSidebar: !0, hasEvent: !0 }, a.formatDate = d.proxy(a.formatDate, a), a.selectDate = d.proxy(a.selectDate, a), a.selectMonth = d.proxy(a.selectMonth, a), a.selectYear = d.proxy(a.selectYear, a), a.selectEvent = d.proxy(a.selectEvent, a), a.toggleSidebar = d.proxy(a.toggleSidebar, a), a.toggleEventList = d.proxy(a.toggleEventList, a), a.instanceUid = i++, a.init(!0)
    }).prototype.init = function(e) {
        var t = this;
        d(t.$elements.calendarEl).hasClass("calendar-initialized") || (d(t.$elements.calendarEl).addClass("abhi-calendar calendar-initialized"), t.windowW <= t.$breakpoints.tablet ? (t.toggleSidebar(!1), t.toggleEventList(!1)) : (t.options.sidebarDisplayDefault ? t.toggleSidebar(!0) : t.toggleSidebar(!1), t.options.eventDisplayDefault ? t.toggleEventList(!0) : t.toggleEventList(!1)), t.options.theme && t.setTheme(t.options.theme), t.buildTheBones())
    }, r.prototype.destroy = function() {
        var e = this;
        e.destroyEventListener(), e.$elements.calendarEl && (e.$elements.calendarEl.removeClass("calendar-initialized"), e.$elements.calendarEl.removeClass("abhi-calendar"), e.$elements.calendarEl.removeClass("sidebar-hide"), e.$elements.calendarEl.removeClass("event-hide")), e.$elements.calendarEl.empty(), e.$elements.calendarEl.attr("class", e.initials.default_class), d(e.$elements.calendarEl).trigger("destroy", [e])
    }, r.prototype.limitTitle = function(e, t) { var a = []; if (t = void 0 === t ? 18 : t, e.split(" ").join("").length > t) { for (var n = e.split(" "), i = 0; i < n.length; i++) n[i].length + a.join("").length <= t && a.push(n[i]); return a.join(" ") + "..." } return e }, r.prototype.stringCheck = function(e) { return e.replace(/[^\w]/g, "\\$&") }, r.prototype.parseFormat = function(e) {
        ...
        var i = new Date(e),
            o = { d: i.getDate(), D: n.initials.dates[a].daysShort[i.getDay()], DD: n.initials.dates[a].days[i.getDay()], m: i.getMonth() + 1, M: n.initials.dates[a].monthsShort[i.getMonth()], MM: n.initials.dates[a].months[i.getMonth()], yy: i.getFullYear().toString().substring(2), yyyy: i.getFullYear() };
        ...
        return a
    }, r.prototype.isBetweenDates = function(e, t) { var a, t = t instanceof Array ? (a = new Date(t[0]), new Date(t[1])) : (a = new Date(t), new Date(t)); return a <= new Date(e) && t >= new Date(e) }, r.prototype.hasSameDayEventType = function(e, t) {
        ...
        var e = this;
        e.windowW = d(window).width(), e.windowW <= e.$breakpoints.tablet ? (e.toggleSidebar(!1), e.toggleEventList(!1), e.windowW <= e.$breakpoints.mobile ? d(window).off("click.abhicalendar.abhi-" + e.instanceUid) : d(window).on("click.abhicalendar.abhi-" + e.instanceUid, d.proxy(e.toggleOutside, e))) : (e.options.sidebarDisplayDefault ? e.toggleSidebar(!0) : e.toggleSidebar(!1), e.options.eventDisplayDefault ? e.toggleEventList(!0) : e.toggleEventList(!1), d(window).off("click.abhicalendar.abhi-" + e.instanceUid))
    }, r.prototype.initEventListener = function() {
        var e = this;
        d(window).off("resize.abhicalendar.abhi-" + e.instanceUid).on("resize.abhicalendar.abhi-" + e.instanceUid, d.proxy(e.resize, e)), e.options.sidebarToggler && e.$elements.sidebarToggler.off("click.abhicalendar").on("click.abhicalendar", e.toggleSidebar), e.options.eventListToggler && e.$elements.eventListToggler.off("click.abhicalendar").on("click.abhicalendar", e.toggleEventList), e.$elements.sidebarEl.find("[data-month-val]").off("click.abhicalendar").on("click.abhicalendar", e.selectMonth), e.$elements.sidebarEl.find("[data-year-val]").off("click.abhicalendar").on("click.abhicalendar", e.selectYear), e.$elements.eventEl.find("[data-event-index]").off("click.abhicalendar").on("click.abhicalendar", e.selectEvent)
    }, r.prototype.destroyEventListener = function() {
        var e = this;
        d(window).off("resize.abhicalendar.abhi-" + e.instanceUid), d(window).off("click.abhicalendar.abhi-" + e.instanceUid), e.options.sidebarToggler && e.$elements.sidebarToggler.off("click.abhicalendar"), e.options.eventListToggler && e.$elements.eventListToggler.off("click.abhicalendar"), e.$elements.innerEl.find(".calendar-day").children().off("click.abhicalendar"), e.$elements.sidebarEl.find("[data-month-val]").off("click.abhicalendar"), e.$elements.sidebarEl.find("[data-year-val]").off("click.abhicalendar"), e.$elements.eventEl.find("[data-event-index]").off("click.abhicalendar")
        ...
        function o(e) { a = !0, t.addEventList(e) }
    ...
    }, r.prototype.buildSidebarYear = function() { this.$elements.sidebarEl.find(".calendar-year > p").text(this.$active.year) }, r.prototype.buildSidebarMonths = function() { this.$elements.sidebarEl.find(".calendar-months > [data-month-val]").removeClass("active-month"), this.$elements.sidebarEl.find('.calendar-months > [data-month-val="' + this.$active.month + '"]').addClass("active-month") }, r.prototype.buildCalendar = function() {
        var e, t = this;
        ...
        for (var a = 1, n = 0; n < 9; n++) {
            for (var i, o = 0; o < t.$label.days.length; o++) { a <= t.monthLength && (0 < n || o >= t.startingDay) ? (i = "calendar-day", t.$label.days[o] !== t.initials.weekends.sat && t.$label.days[o] !== t.initials.weekends.sun || (i += " --weekend"), e += '<td class="' + i + '">', e += '<div class="day" role="button" data-date-val="' + t.formatDate(t.$label.months[t.$active.month] + " " + a + " " + t.$active.year, t.options.format) + '">' + a + "</div>", a++) : e += "<td>", e += "</td>" }
            if (a > t.monthLength) break;
            e += '</tr><tr class="calendar-body">'
        }
        e += "</tr>", t.$elements.innerEl.find(".calendar-table").append(e), t.options.todayHighlight && t.$elements.innerEl.find("[data-date-val='" + t.$current.date + "']").addClass("calendar-today"), t.$elements.innerEl.find(".calendar-day").children().off("click.abhicalendar").on("click.abhicalendar", t.selectDate);
        ...
        function d(e) { 0 === (n = i.$elements.innerEl.find('[data-date-val="' + e + '"]')).find("span.event-indicator").length && n.append('<span class="event-indicator"></span>'), 0 === n.find("span.event-indicator > .type-bullet > .type-" + o).length && (a = '<div class="type-bullet"><div ', a += 'class="type-' + t.type + '"', t.color && (a += 'style="background-color:' + t.color + '"'), a += "></div></div>", n.find(".event-indicator").append(a)) }
    ...
    }, r.prototype.buildEventIndicator = function() { this.$elements.innerEl.find(".calendar-day > day > .event-indicator").empty(); for (var e = 0; e < this.options.calendarEvents.length; e++) this.addEventIndicator(this.options.calendarEvents[e]) }, r.prototype.selectEvent = function(e) {
        var t = this,
            a = d(e.target).closest(".event-container"),
            e = d(a).data("eventIndex").toString(),
            a = t.options.calendarEvents.map(function(e) { return e.id.toString() }).indexOf(e),
    ...
    }, r.prototype.getActiveDate = function() { return this.$active.date }, r.prototype.getActiveEvents = function() { return this.$active.events }, r.prototype.toggleOutside = function(e) {
        var t = this,
        ...
        function t(e) {
            var t, a = n.options.calendarEvents.map(function(e) { return e.id }).indexOf(e);
    ...
    }, r.prototype.isValidDate = function(e) { return new Date(e) && !isNaN(new Date(e).getTime()) }, d.fn.abhiCalendar = function() {
        for (var e, t = this, a = arguments[0], n = Array.prototype.slice.call(arguments, 1), i = t.length, o = 0; o < i; o++)
            if ("object" == typeof a || void 0 === a ? t[o].abhiCalendar = new r(t[o], a) : e = t[o].abhiCalendar[a].apply(t[o].abhiCalendar, n), void 0 !== e) return e;
        return t
    }
});